...
$$
theta = P(X_1 < X_2) + \frac{1}{2} P(X_1 = X_2).
$$

If lower values are favorable, $theta$ can be interpreted as the probability that
...
expressed on the win odds scale, $theta / (1 - theta)$, or on the log win odds scale,
$\log\{theta / (1 - theta)\}$. These effect scales are particularly attractive for
...
- Natural effect scale: Mann-Whitney parameter $theta$
- Typical null hypothesis: equality of distributions, or operationally
  $theta = 1/2$ under additional assumptions
...
- Allows testing $H_0: theta = 1/2$ without requiring equality of the full
  distributions
...
- Natural effect scale: log win odds, $\log\{theta / (1 - theta)\}$
...
- Stage-wise estimate of the Mann-Whitney parameter $\hat theta_k$
