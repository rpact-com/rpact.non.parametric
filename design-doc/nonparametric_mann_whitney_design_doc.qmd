...
$$
\theta = P(X_1 < X_2) + \frac{1}{2} P(X_1 = X_2).
$$

If lower values are favorable, $\theta$ can be interpreted as the probability that
...
expressed on the win odds scale, $\theta / (1 - \theta)$, or on the log win odds scale,
$\log\{\theta / (1 - \theta)\}$. These effect scales are particularly attractive for
...
- Natural effect scale: Mann-Whitney parameter $\theta$
- Null hypothesis: equality of distributions $H_0: F_1 = F_2$
...
- Natural effect scale: Mann-Whitney parameter $\theta$
- Targets the nonparametric Behrens-Fisher problem
- Null hypothesis: $H_0: \theta = 1/2$ without requiring equality of the full
  distributions
...
- Natural effect scale: log win odds, $\log\{\theta/(1-\theta)\}$
- Can be mapped back to win odds or the Mann-Whitney parameter for
  interpretation
- Null hypothesis: $H_0: \log\{\theta/(1-\theta)\} = 0$
- May be useful as the default analysis scale as it provides range-preserving
...
getSampleSizeNonparametric(
    ...
    endpointScale = c("continuous", "ordinal"),
...
- Should `effect` always be interpreted according to `effectMeasure`, or should
  separate arguments such as `theta`, `winOdds`, and `logWinOdds` be preferred?
...
- Stage-wise estimate of the Mann-Whitney parameter $\hat \theta_k$
